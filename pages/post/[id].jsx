...
export default function PostDetail() {
  ...
  useEffect(() => {
    ...
    getPostById(id)
      .then((response) => {
        setPost(response); // Asumiendo que `getPostById` devuelve el post completo
      })
      .catch((error) => {
        console.log("[getPostById] error:", error);
        setError("Failed to load post."); // Manejo del error
      });
  }, [id]);

  if (error) return <p className="text-red-500">{error}</p>; // Mostrar error
  if (!post) return <p className="text-black">Loading...</p>;
